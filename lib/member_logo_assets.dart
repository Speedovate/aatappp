import 'package:flutter/material.dart';

class MemberLogoAsset {
  final String associationName;
  final String? officialBrandName;
  final String category;
  final String type;
  final String? officialUrl;
  final String? logoSourceUrl;
  final String? assetPath;
  final String verificationStatus;
  final String? notes;

  const MemberLogoAsset({
    required this.associationName,
    this.officialBrandName,
    required this.category,
    required this.type,
    this.officialUrl,
    this.logoSourceUrl,
    this.assetPath,
    required this.verificationStatus,
    this.notes,
  });
}

class MemberLogo extends StatelessWidget {
  const MemberLogo({
    super.key,
    required this.member,
    this.maxWidth = 180,
    this.maxHeight = 96,
    this.padding = const EdgeInsets.all(18),
  });

  final MemberLogoAsset member;
  final double maxWidth;
  final double maxHeight;
  final EdgeInsets padding;

  @override
  Widget build(BuildContext context) {
    final hasAsset = member.assetPath != null;
    final showDarkBackground =
        member.assetPath != null &&
        (member.assetPath!.endsWith('.webp') ||
            member.assetPath!.contains('funny-lion'));

    return Container(
      constraints: BoxConstraints(maxWidth: maxWidth, minHeight: maxHeight),
      padding: padding,
      decoration: BoxDecoration(
        color: showDarkBackground ? Colors.black : Colors.white,
        border: Border.all(color: Colors.black.withValues(alpha: 0.08)),
        borderRadius: BorderRadius.circular(20),
      ),
      child: Center(
        child: hasAsset
            ? Image.asset(
                member.assetPath!,
                fit: BoxFit.contain,
                errorBuilder: (context, error, stackTrace) {
                  return _MemberLogoFallback(member: member);
                },
              )
            : _MemberLogoFallback(member: member),
      ),
    );
  }
}

class _MemberLogoFallback extends StatelessWidget {
  const _MemberLogoFallback({required this.member});

  final MemberLogoAsset member;

  @override
  Widget build(BuildContext context) {
    return Text(
      member.officialBrandName ?? member.associationName,
      textAlign: TextAlign.center,
      style: const TextStyle(
        fontSize: 14,
        fontWeight: FontWeight.w700,
        height: 1.4,
      ),
    );
  }
}

const memberLogoAssets = [
  MemberLogoAsset(
    associationName: 'A&A Plaza Hotel',
    officialBrandName: 'A&A Plaza Hotel',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://www.facebook.com/aaplazahotel/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSsVaX7Oo_qAeQ83RfscY75EfsRr30U4qGJ7-JodkqgiaTdNaDjBozVN0RE&s=10',
    assetPath: 'assets/aatappp/members/hotels_resorts/aa-plaza-hotel.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Astoria Palawan',
    officialBrandName: 'Astoria Palawan',
    category: 'Hotels & Resorts',
    type: 'Resort / Hotel',
    officialUrl: 'https://astoriapalawan.com/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRRJaxpsRGTqt0NOoXTN65o0Q1HrQ9kksNmc2H1guXZ8QtkkEwYFaZTAnY&s=10',
    assetPath:
        'assets/aatappp/members/hotels_resorts/astoria-palawan-user-source.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Aziza Paradise Hotel',
    officialBrandName: 'Aziza Paradise Hotel',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://www.azizaparadisehotel.ph/',
    logoSourceUrl:
        'https://www.azizaparadisehotel.ph/wp-content/uploads/2024/06/logo-aziza-1.jpg',
    assetPath: 'assets/aatappp/members/hotels_resorts/aziza-paradise-hotel.jpg',
    verificationStatus: 'verified_official_website',
    notes:
        'Official site exposed a raster logo file rather than a transparent PNG or SVG.',
  ),
  MemberLogoAsset(
    associationName: 'Best Western Plus the Ivywall Hotel',
    officialBrandName: 'Best Western Plus the Ivywall Hotel',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl:
        'https://www.bestwestern.com/en_US/book/hotels-in-puerto-princesa/best-western-plus-the-ivywall-hotel-palawan/propertyCode.99104.html',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTFJgJUZSzQWmXvWHNdK9TOznT7Fs-YvD_RsHm52A7Y9ic6sQ0_tFHnc-U&s=10',
    assetPath:
        'assets/aatappp/members/hotels_resorts/best-western-plus-the-ivywall-hotel.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Citystate Asturias Hotel',
    officialBrandName: 'Citystate Asturias Hotel',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://www.facebook.com/citystateasturias/',
    logoSourceUrl:
        'https://alcgroup.com.ph/assets/citystate_austrias_hotel-D3C3n_Qu.png',
    assetPath:
        'assets/aatappp/members/hotels_resorts/citystate-asturias-hotel.png',
    verificationStatus: 'verified_official_website',
    notes:
        'Downloaded from the broader ALC Group property-controlled website asset.',
  ),
  MemberLogoAsset(
    associationName: 'Costa Palawan Resort',
    officialBrandName: 'Costa Palawan Resort',
    category: 'Hotels & Resorts',
    type: 'Resort',
    officialUrl: 'https://costapalawanresort.com/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcThBsWBrTtDdCTiA-XB697YQMQjS5EqsAzGQtupSDhUxdKO1bSMcz-qcyk&s=10',
    assetPath:
        'assets/aatappp/members/hotels_resorts/costa-palawan-resort-user-source.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Daluyon Beach and Mountain Resort',
    officialBrandName: 'Daluyon Beach and Mountain Resort',
    category: 'Hotels & Resorts',
    type: 'Beach / Mountain Resort',
    officialUrl: 'https://daluyonbeachandmountainresort.com/',
    logoSourceUrl:
        'https://daluyonbeachandmountainresort.com/wp-content/themes/yootheme/cache/Daluyon-logo-No-BG_19617112058-e1570772333736-e2768099.png',
    assetPath:
        'assets/aatappp/members/hotels_resorts/daluyon-beach-and-mountain-resort.png',
    verificationStatus: 'verified_official_website',
  ),
  MemberLogoAsset(
    associationName: 'Fersal Hotel',
    officialBrandName: 'Fersal Hotel',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl:
        'https://fersalhotel.com/fersal-hotel-puerto-princesa-city-palawan/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmgc3Mdw994o0Cg4kjbbUbiHplPLO_116QkUuBHuRDS82QuDv64nFyseo&s=10',
    assetPath: 'assets/aatappp/members/hotels_resorts/fersal-hotel.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Four Points by Sheraton Palawan',
    officialBrandName: 'Four Points by Sheraton Palawan',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl:
        'https://www.marriott.com/en-us/hotels/ppsfp-four-points-palawan-puerto-princesa/overview/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRR-6mCpkYupyf3eDaXJ1mt0j3OLjtWtGKIsPFYic1KaVVSXG8-aOXRyXc&s=10',
    assetPath:
        'assets/aatappp/members/hotels_resorts/four-points-by-sheraton-palawan.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source that matches Four Points branding.',
  ),
  MemberLogoAsset(
    associationName: 'Go Hotels',
    officialBrandName: 'Go Hotels',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://gohotels.ph/branch/puerto-princesa',
    logoSourceUrl:
        'https://gohotels.ph/themes/custom/rhr_gohotels/images/logo/logo-green.png',
    assetPath:
        'assets/aatappp/members/hotels_resorts/go-hotels-puerto-princesa.png',
    verificationStatus: 'verified_official_website',
    notes:
        'Downloaded from the official Go Hotels branch site; chain brand mark rather than a branch-specific lockup.',
  ),
  MemberLogoAsset(
    associationName: 'Holiday Suites',
    officialBrandName: 'Holiday Suites',
    category: 'Hotels & Resorts',
    type: 'Suites Hotel',
    officialUrl: 'https://holidaysuites.ph/',
    logoSourceUrl: 'https://holidaysuites.ph/img/hs-logo.png',
    assetPath:
        'assets/aatappp/members/hotels_resorts/holiday-suites-puerto-princesa.png',
    verificationStatus: 'verified_official_website',
  ),
  MemberLogoAsset(
    associationName: 'Microtel by Wyndham',
    officialBrandName: 'Microtel by Wyndham Puerto Princesa',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://microtel-palawan.com/',
    logoSourceUrl:
        'https://ireward.superghs.com/resource/microtelpuertoprincesa/logo/logo.png?v1.0004',
    assetPath: 'assets/aatappp/members/hotels_resorts/microtel-by-wyndham.png',
    verificationStatus: 'verified_official_website',
  ),
  MemberLogoAsset(
    associationName: 'Palawan Uno Hotel',
    officialBrandName: 'Palawan Uno Hotel',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://www.facebook.com/palawanunohotel/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT-tAl7a2sOExybWsyv8WuidW3uDJYo8-ce8we-n4klMcZ68JieTRl8vSA&s=10',
    assetPath: 'assets/aatappp/members/hotels_resorts/palawan-uno-hotel.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Sotogrande Palawan Hotel',
    officialBrandName: 'Sotogrande Palawan Hotel',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://www.facebook.com/officialsotograndepalawan/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSf5_FLC6Ygw-UZt7wpC0-yCGB-6xOYLHBcK8KHk0Alp5-yoBrp4Lr9XttU&s=10',
    assetPath:
        'assets/aatappp/members/hotels_resorts/sotogrande-palawan-hotel.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Sunlight Guest Hotel Inc.',
    officialBrandName: 'Sunlight Guest Hotel Inc.',
    category: 'Hotels & Resorts',
    type: 'Guest Hotel',
    officialUrl:
        'https://www.facebook.com/pages/Sunlight-Guest-Hotel-Puerto-Princesa-Palawan/310557065634916',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTlb77aE7ZyyDa2FL59s_QhO4sA5VW0VSi4zRZRgac-_vqmopfFJrlFQM-3&s=10',
    assetPath:
        'assets/aatappp/members/hotels_resorts/sunlight-guest-hotel-inc.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'The Dome Palawan',
    officialBrandName: 'The Dome Palawan',
    category: 'Hotels & Resorts',
    type: 'Hotel / Resort',
    officialUrl: 'https://www.facebook.com/thedomepuertoprincesa/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR3YVorZsWh6x54l3Ex9a9Pi3uFe9QUZcpDhWkOTzjURmLmxC45I3cvZuhf&s=10',
    assetPath: 'assets/aatappp/members/hotels_resorts/the-dome-palawan.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source that matches The Dome branding.',
  ),
  MemberLogoAsset(
    associationName: 'The Funny Lion',
    officialBrandName: 'The Funny Lion',
    category: 'Hotels & Resorts',
    type: 'Hotel / Resort',
    officialUrl: 'https://www.thefunnylion.com/',
    logoSourceUrl:
        'https://images.squarespace-cdn.com/content/v1/633baf35752b3571b1938cdf/b2effb2b-37f4-40d5-b572-186a3752eae7/The+Funny+Lion+white.png?format=1500w',
    assetPath:
        'assets/aatappp/members/hotels_resorts/the-funny-lion-puerto-princesa.webp',
    verificationStatus: 'verified_official_website',
    notes:
        'Downloaded from the official site header logo URL. Returned as WebP.',
  ),
  MemberLogoAsset(
    associationName: 'Princesa Garden Island Resort',
    officialBrandName: 'Princesa Garden Island Resort',
    category: 'Hotels & Resorts',
    type: 'Resort',
    officialUrl: 'https://www.princesagardenisland.com/',
    logoSourceUrl:
        'https://www.princesagardenisland.com/wp-content/uploads/2024/06/main-logo-1.png',
    assetPath:
        'assets/aatappp/members/hotels_resorts/princesa-garden-island-resort.png',
    verificationStatus: 'verified_official_website',
  ),
  MemberLogoAsset(
    associationName: 'Crown Hotel Palawan at Harbour Springs',
    officialBrandName: 'Crown Hotel Palawan',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://crownhotelpalawan.com/',
    logoSourceUrl:
        'https://crownhotelpalawan.com/wp-content/uploads/2015/08/cropped-300x200-v1.png',
    assetPath:
        'assets/aatappp/members/hotels_resorts/crown-hotel-palawan-at-harbour-springs.png',
    verificationStatus: 'verified_official_website',
    notes:
        'Updated to the user-confirmed Crown Hotel logo image from the official website.',
  ),
  MemberLogoAsset(
    associationName: 'Hotel Centro',
    officialBrandName: 'Hotel Centro',
    category: 'Hotels & Resorts',
    type: 'Hotel',
    officialUrl: 'https://www.facebook.com/HotelCentro/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTAvH203fbjujZCbD7Cq77eJL1sVErrLvExmkJD7W7bJtzK8XvFT8HaPwg&s=10',
    assetPath: 'assets/aatappp/members/hotels_resorts/hotel-centro.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Ala Amid Bed & Breakfast',
    officialBrandName: 'Ala Amid Bed & Breakfast',
    category: 'Mabuhay Accommodations',
    type: 'Bed & Breakfast',
    officialUrl: 'https://www.facebook.com/alaamidbedandbreakfast/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6s3GPpUG1oz7N0WttPePLtBW8gERTrW-yTxoEzs2xYZw9KRSTP1EE8f6H&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/ala-amid-bed-and-breakfast.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Alvea Hotel',
    officialBrandName: 'Alvea Hotel',
    category: 'Mabuhay Accommodations',
    type: 'Hotel',
    officialUrl: 'https://www.alvea.com.ph/',
    logoSourceUrl:
        'https://www.alvea.com.ph/wp-content/uploads/2022/05/logo.png',
    assetPath: 'assets/aatappp/members/mabuhay/alvea-hotel-puerto-princesa.png',
    verificationStatus: 'verified_official_website',
  ),
  MemberLogoAsset(
    associationName: 'Angelic Mansion',
    officialBrandName: 'Angelic Mansion',
    category: 'Mabuhay Accommodations',
    type: 'Mansion / Accommodation',
    officialUrl: 'https://www.angelicmansion.com/',
    logoSourceUrl:
        'https://s3-cdn.hotellinksolutions.com/hls/data/1033/website/general/lg/normal_angelic-mansion-logo.png',
    assetPath: 'assets/aatappp/members/mabuhay/angelic-mansion.png',
    verificationStatus: 'verified_official_website',
  ),
  MemberLogoAsset(
    associationName: 'Bambu Suites',
    officialBrandName: 'Bambu Suites',
    category: 'Mabuhay Accommodations',
    type: 'Suites',
    officialUrl: 'https://www.facebook.com/bambusuitespalawan/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSrpF12dID-9SR_fFL9Fk-fVAKssQtjCwnMNS-eTTMOiZ57R9AZ-2yL27Y&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/bambu-suites.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Blue Lagoon Inn & Suites',
    officialBrandName: 'Blue Lagoon Inn & Suites',
    category: 'Mabuhay Accommodations',
    type: 'Inn / Suites',
    officialUrl:
        'https://www.facebook.com/p/Blue-Lagoon-Inn-Suites-100064013111248/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT5uqUZy904gpTxvNuhnhH8JewJmm_sOzgtTvHv4BUxi7OjtVdF4DxdQ5vJ&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/blue-lagoon-inn-and-suites.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Cafemoto Lifestyle',
    officialBrandName: 'Cafemoto Lifestyle',
    category: 'Mabuhay Accommodations',
    type: 'Lifestyle Accommodation',
    officialUrl: 'https://www.facebook.com/onehundredpercentcafemoto/',
    logoSourceUrl:
        'https://a.hwstatic.com/image/upload/f_auto,q_auto,t_40/propertyimages/3/326520/q58ilw8soexxxbqtngii.jpg',
    assetPath: 'assets/aatappp/members/mabuhay/cafemoto-lifestyle.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Canvas Boutique Hotel',
    officialBrandName: 'Canvas Boutique Hotel',
    category: 'Mabuhay Accommodations',
    type: 'Boutique Hotel',
    officialUrl: 'https://canvasboutiquehotel.com/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSUlXoh6flO1E99WaCBhHIlnQsNaW_JbU5vOi9L5sx-pDjrw1RExOqsKeU&s=10',
    assetPath:
        'assets/aatappp/members/mabuhay/canvas-boutique-hotel-user-source.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Casa Belina Tourist Inn',
    officialBrandName: 'Casa Belina Tourist Inn',
    category: 'Mabuhay Accommodations',
    type: 'Tourist Inn',
    officialUrl: 'https://www.facebook.com/casabelinatouristinn/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTHK6mRUEKFH0DUIVhRMckORmvmBd_q7dgA7nRETZwjsnK1WLQXC1MJGuY_&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/casa-belina-tourist-inn.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Casa De Praxides Tourist Inn',
    officialBrandName: 'Casa De Praxides Tourist Inn',
    category: 'Mabuhay Accommodations',
    type: 'Tourist Inn',
    officialUrl:
        'https://www.facebook.com/p/Casa-de-Praxides-Tourist-Inn-61551913493742/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTrkaumEMaNIPXc5muiZRVFiHymMSM5h-Ph0vZYSSD-umdMnu7gdEA018Ec&s=10',
    assetPath:
        'assets/aatappp/members/mabuhay/casa-de-praxides-tourist-inn.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Casañas Suites',
    officialBrandName: 'Casañas Suites',
    category: 'Mabuhay Accommodations',
    type: 'Suites',
    officialUrl: 'https://www.facebook.com/casanassuitepalawan/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSkURYJNC7eHbSyPmp3-OGJobNliRZrsVmGm7DfmkSoieuh0ZOBsNcq6_4&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/casanas-suites.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: "D' Beach Resort",
    officialBrandName: "D' Beach Resort",
    category: 'Mabuhay Accommodations',
    type: 'Beach Resort',
    officialUrl: 'https://www.facebook.com/DBeachResortPuertoPrincesa/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTBA7B46iL2WemfZgnPOEWG9eP1c6WUmbIBNzgElrnmRVh8OkJAzRUDWjA&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/d-beach-resort.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Diakopes Inn',
    officialBrandName: 'Diakopes Inn',
    category: 'Mabuhay Accommodations',
    type: 'Inn',
    officialUrl: 'https://www.facebook.com/diakopesinn/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSLsZxR7eSlut_tiD4iE1NFxslZxiAZmxOwk8_Uubs1F_c992h2QQ6kPLSg&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/diakopes-inn.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Dinah’s Tourist Inn',
    category: 'Mabuhay Accommodations',
    type: 'Tourist Inn',
    officialUrl: 'https://dinahspensionhouse.zoombookdirect.com/',
    verificationStatus: 'needs_manual_review',
    notes:
        'Property-controlled source suggests Dinah’s Pension House branding; discrepancy recorded, no safe logo download in this pass.',
  ),
  MemberLogoAsset(
    associationName: 'Dolce Vita Hotel & Restaurant',
    category: 'Mabuhay Accommodations',
    type: 'Hotel',
    officialUrl:
        'https://www.facebook.com/p/Dolce-Vita-Hotel-Restaurant-100064061413758/',
    verificationStatus: 'needs_manual_review',
    notes:
        'Facebook-only source; no confidently extracted official brand asset in this pass.',
  ),
  MemberLogoAsset(
    associationName: 'Elleis Place',
    officialBrandName: 'Elleis Place',
    category: 'Mabuhay Accommodations',
    type: 'Accommodation',
    officialUrl: 'https://www.facebook.com/elleisplace/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS5xvgPNSN1s3DNvS1IHVyM7LEeqo4q13O0yvkHPu96xw&s',
    assetPath: 'assets/aatappp/members/mabuhay/elleis-place.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Escape Boutique Hotel',
    officialBrandName: 'Escape Boutique Hotel',
    category: 'Mabuhay Accommodations',
    type: 'Boutique Hotel',
    officialUrl:
        'https://www.facebook.com/p/Escape-Boutique-Hotel-by-FarmBihira-Farm-61553132618395/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSzqT9PxY6PmDjNgcRWnGZqwXXsT7-pxdVvn-1lhqNzX7xhUXluaqOTT2al&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/escape-boutique-hotel.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Grande Vista Hotel',
    officialBrandName: 'Grande Vista Hotel',
    category: 'Mabuhay Accommodations',
    type: 'Hotel',
    officialUrl: 'https://www.facebook.com/Gvh12/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRVGsOOFecN5NXy7OzNM-r8NYbLITOuu586hwZ_kYMuSw&s',
    assetPath: 'assets/aatappp/members/mabuhay/grande-vista-hotel.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Hotel Palacio',
    officialBrandName: 'Hotel Palacio',
    category: 'Mabuhay Accommodations',
    type: 'Hotel',
    officialUrl: 'https://www.hotelpalaciopalawan.com/',
    logoSourceUrl:
        'https://www.hotelpalaciopalawan.com/images/uploads/logos/2698_logo.jpg',
    assetPath: 'assets/aatappp/members/mabuhay/hotel-palacio.jpg',
    verificationStatus: 'verified_official_website',
    notes: 'Downloaded from the property header logo on the official website.',
  ),
  MemberLogoAsset(
    associationName: 'KSK Serenity Resort',
    officialBrandName: 'KSK Serenity Resort',
    category: 'Mabuhay Accommodations',
    type: 'Resort',
    officialUrl:
        'https://www.facebook.com/p/KSK-Serenity-Resort-61557125019785/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQjBiHcOvTVbY22wAkb46c01UnuI3wrJ1Xfw1gswhCvmJjy18Hiafxv-g0&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/ksk-serenity-resort.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Lokal Hut Bed & Breakfast',
    officialBrandName: 'Lokal Hut Bed & Breakfast',
    category: 'Mabuhay Accommodations',
    type: 'Bed & Breakfast',
    officialUrl: 'https://www.facebook.com/p/The-Lokal-Hut-61556669164959/',
    logoSourceUrl:
        'https://cf.bstatic.com/xdata/images/hotel/max1024x768/541894332.jpg?k=76aadf1e4be7e9b56b64184a051b7587b5357cecca33d91fbafdca98e22c384b&o=',
    assetPath: 'assets/aatappp/members/mabuhay/lokal-hut-bed-and-breakfast.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Lola Itangs Pension House',
    officialBrandName: 'Lola Itangs Pension House',
    category: 'Mabuhay Accommodations',
    type: 'Pension House',
    officialUrl: 'https://www.facebook.com/lolaitangpalawan/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR4F8TZYCmWcshAGW9eoEIBcoQw_pMskQzyCYSFbC1-v5dPPwspCCjNj34&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/lola-itangs-pension-house.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Orange Mangrove Pension House',
    category: 'Mabuhay Accommodations',
    type: 'Pension House',
    officialUrl: 'https://www.orangemangrovehotel.com/',
    verificationStatus: 'needs_manual_review',
    notes:
        'Official source suggests Orange Mangrove Hotel branding, but direct logo retrieval was not confidently completed in this pass.',
  ),
  MemberLogoAsset(
    associationName: 'One Eight Residences Inn',
    officialBrandName: 'One Eight Residence Inn',
    category: 'Mabuhay Accommodations',
    type: 'Residences / Inn',
    officialUrl: 'https://oneeightresidenceinn.com/',
    logoSourceUrl:
        'https://oneeightresidenceinn.com/wp-content/uploads/2021/05/18-logo-flat.png',
    assetPath: 'assets/aatappp/members/mabuhay/one-eight-residence-inn.png',
    verificationStatus: 'verified_official_website',
    notes:
        'Association listing uses One Eight Residences Inn; official site uses One Eight Residence Inn.',
  ),
  MemberLogoAsset(
    associationName: 'Puerto Pension Inn',
    officialBrandName: 'Puerto Pension Inn',
    category: 'Mabuhay Accommodations',
    type: 'Pension House / Inn',
    officialUrl: 'https://www.facebook.com/puertopensioninn/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQuhDcDXly5xamNEItlFbJVRxd55r8GR2CVryjksNnYds5T9aQXoa6ff8JI&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/puerto-pension-inn.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Rodolfo Royale Hotel',
    officialBrandName: 'Rodolfo Royale Hotel',
    category: 'Mabuhay Accommodations',
    type: 'Hotel',
    officialUrl:
        'https://www.facebook.com/p/Rodolfo-Royale-HOTEL-61571256355874/',
    logoSourceUrl:
        'https://images.puertoparrot.com/filemanager/photos/3/PP-Rodolfo-Royale/logo.png',
    assetPath: 'assets/aatappp/members/mabuhay/rodolfo-royale-hotel.png',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Sandy Beach Hotel and Resort',
    category: 'Mabuhay Accommodations',
    type: 'Hotel / Resort',
    verificationStatus: 'missing',
    notes:
        'No sufficiently verified official source was available in the supplied manifest; intentionally left without a downloaded logo.',
  ),
  MemberLogoAsset(
    associationName: 'Sheridan Boutique Hotel',
    officialBrandName: 'Sheridan Boutique Hotel',
    category: 'Mabuhay Accommodations',
    type: 'Boutique Hotel',
    officialUrl: 'https://sheridanboutiquehotel.com/',
    logoSourceUrl:
        'https://sheridanboutiquehotel.com/wp-content/uploads/2018/06/sheridan-boutique-logo-300x225.png',
    assetPath: 'assets/aatappp/members/mabuhay/sheridan-boutique-hotel.png',
    verificationStatus: 'verified_official_website',
    notes: 'Downloaded from the hotel header logo on the official website.',
  ),
  MemberLogoAsset(
    associationName: 'Southwind Palawan',
    officialBrandName: 'Southwind Palawan',
    category: 'Mabuhay Accommodations',
    type: 'Accommodation',
    officialUrl: 'https://www.facebook.com/southwindpalawan/',
    logoSourceUrl:
        'https://images.puertoparrot.com/filemanager/photos/3/PP-Southwind-Palawan/17265163_395966150762594_4496462318098601989_n.gif',
    assetPath: 'assets/aatappp/members/mabuhay/southwind-palawan.gif',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Upgrace Inn',
    officialBrandName: 'Upgrace Inn',
    category: 'Mabuhay Accommodations',
    type: 'Inn',
    officialUrl: 'https://www.facebook.com/UpgraceInn/',
    logoSourceUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSJn6qGto856D3YuxsP7sdo4dNw3bvdujJqao6SzF8vgyT-JMxbC1TFI4v1&s=10',
    assetPath: 'assets/aatappp/members/mabuhay/upgrace-inn.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
  MemberLogoAsset(
    associationName: 'Wanderlust Bed & Breakfast',
    officialBrandName: 'Wanderlust',
    category: 'Mabuhay Accommodations',
    type: 'Bed & Breakfast',
    officialUrl: 'https://wanderlustpalawan.com/',
    logoSourceUrl:
        'https://wanderlustpalawan.com/wp-content/uploads/2026/07/Logo-Font-e1677468406890.png',
    assetPath:
        'assets/aatappp/members/mabuhay/wanderlust-bed-and-breakfast.png',
    verificationStatus: 'verified_official_website',
  ),
  MemberLogoAsset(
    associationName: 'Whitebreeze Palawan Hotel',
    officialBrandName: 'Whitebreeze Palawan Hotel',
    category: 'Mabuhay Accommodations',
    type: 'Hotel',
    officialUrl: 'https://www.facebook.com/whitebreezepalawan/',
    logoSourceUrl:
        'https://s7ap1.scene7.com/is/image/bpi/hero-white-breeze-palawan?qlt=85&wid=1024&ts=1764054211639&dpr=off',
    assetPath: 'assets/aatappp/members/mabuhay/whitebreeze-palawan-hotel.jpg',
    verificationStatus: 'user_provided_source',
    notes:
        'Uses the user-provided external image source for the current member pill artwork.',
  ),
];
